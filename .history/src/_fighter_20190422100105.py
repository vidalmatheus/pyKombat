...
class Fighter:
    ...
    def fight(self, time, nextFrame):
        ...
        if not self.jumping:
            ...
            # combatMoves = [["j","n","k","m","l","u","f"],["1","4","2","5","3","0","6"]] -> jab
            elif ((keyPressed(self.combat[0]) and self.end_Apunch) or ( and not self.end_Apunch) ) and (not self.hit) : 
                print("flag!")
            ...
            elif ((keyPressed("p") and self.end_Apunch) or ( not keyPressed("p") and not self.end_Apunch) ) and (not self.hit) and () : 
                print("flag!")
                self.curr_sprite = self.spriteList[self.Apunch]
                self.Apunching = self.setState()
                self.setEndState() 
